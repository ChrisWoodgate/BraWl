...
for sweep in sweeps:
    ...
    for window in range(len(windows)):
        for overlap in range(len(overlaps)):
            one_sums_err[window, overlap] = (
                np.std(one_sums[:, window, overlap, :]) /
                np.sqrt(len(methods) * repeats)
            )
            one_sums_mean[window, overlap] = np.mean(one_sums[:, window, overlap, :])
        ...
        for overlap in range(len(overlaps)):
            for walker in range(len(walkers)):
                efficiency[method, walker, :, overlap] = (
                    one_sums_mean[0, overlap] / sums_mean[method, walker, :, overlap]
                )
                efficiency_err[method, walker, :, overlap] = efficiency[
                    method, walker, :, overlap
                ] * np.sqrt(
                    (one_sums_err[0, overlap] / one_sums_mean[0, overlap]) ** 2
                    + (sums_err[method, walker, :, overlap] /
                       sums_mean[method, walker, :, overlap]) ** 2
                )

    cores = np.linspace(
        1, int(np.max(windows) * np.max(walkers)),
        int(np.max(windows) * np.max(walkers))
    )
