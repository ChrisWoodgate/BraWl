...
def create_directories_and_copy_files(brawl_type, source_dir, num_walkers):
    ...
    for window in windows:
      for i in range(3):
          ...
          dir_name = f'{brawl_type}_{num_walkers:02d}_{window:02d}_{i+1}'
          ...
          for j in range(len(lines)):
            if lines[j].strip().startswith("num_windows="):
                ...
                lines[j] = f"performance={brawl_type}\n"
...
source_dir = '.'
num_walkers = 16
create_directories_and_copy_files(brawl_type, source_dir, num_walkers)
